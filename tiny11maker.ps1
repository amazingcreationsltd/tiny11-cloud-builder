...
# -------- PATCH: Ensure install.esd exists (WIM → ESD compatibility) --------

$SourcesPath = "$ScratchDisk\tiny11\sources"
$ESDPath = "$SourcesPath\install.esd"
$WIMPath = "$SourcesPath\install.wim"

if (-not (Test-Path $ESDPath)) {
    if (Test-Path $WIMPath) {
        Write-Output "install.esd not found. Converting install.wim to install.esd..."

        # Auto-select first valid image index (CI-safe)
        $imageIndex = (Get-WindowsImage -ImagePath $WIMPath | Select-Object -First 1).ImageIndex
        ...
        if (-not (Test-Path $ESDPath)) {
            Write-Error "WIM → ESD conversion failed. Aborting."
            exit 1
        }
        ...
        Write-Error "Neither install.esd nor install.wim found. Invalid ISO."
        exit 1
    }
}
